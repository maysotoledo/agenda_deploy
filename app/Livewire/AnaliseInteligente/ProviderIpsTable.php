...
class ProviderIpsTable extends TableComponent
{
    public array $rows = [];
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('connection_type')->label('Tipo')->badge()->sortable(),
            ])
    ...
    private function buildPaginator(?string $search = null): LengthAwarePaginator
    {
        ...
        if ($search !== '') {
            ...
            $filtered = array_values(array_filter($filtered, function ($row) use ($needle) {
                ...
                $type = mb_strtolower((string) data_get($row, 'connection_type', ''));
