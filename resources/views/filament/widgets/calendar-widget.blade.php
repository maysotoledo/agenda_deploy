...
<x-filament-widgets::widget wire:poll.10s="pollCalendarForChanges">
    <x-filament::section>
        @php
            $headerActions = collect($this->getCachedHeaderActions())
                ->reject(fn ($action) => $action->getName() === 'create')
                ->all();
        @endphp

        <div class="flex justify-end flex-1 mb-4">
            <x-filament::actions :actions="$headerActions" class="shrink-0" />
        </div>
